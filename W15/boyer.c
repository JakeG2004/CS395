...
void HorspoolMatching(char* needle, char* haystack, int needleLen, int haystackLen)
{
    ...
    while(i < haystackLen)
    {
        ...
        // Compute d1 using max(t1(c) - k, 1)
        //int d1 = (myArr[(unsigned char)(haystack[i])] - k > 1) ? myArr[(unsigned char)(haystack[i])] - k : 1;
        int d1 = 1;
         if((myArr[(unsigned char)haystack[i]] - k) > 1)
         {
            d1 = myArr[(unsigned char)haystack[i]] - k;
         }
         int d2 = goodArr[k]; // Retrieve corresponding d2 from good suffix table

          // Determine shift distance
        int shift;
        if (k == 0) {
            shift = d1;
        } else if (needleLen > k && k > 0) {
            shift = (d1 > d2) ? d1 : d2;
            printf("%*s%s d1=%d d2=%d\n", i - needleLen + 1, "", needle, d1, d2);
        } else if (k == needleLen) {
            shift = 1;
            printf("%*s%s---found\n", i - needleLen + 1, "", needle);
            matches[i - needleLen + 1] = 1; // Store correct match location
        }
